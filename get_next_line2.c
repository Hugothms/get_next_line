...
/*   Updated: 2019/11/18 10:07:47 by hthomas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	*line = "\0";
	...
	while (i--)
	{
		eol = find_pos_eol(overflow[fd]);
		ft_strlcat_malloc(line, overflow[fd], eol + 1);
		if (ft_strlen(overflow[fd]) != eol)
		{
			set_static_str(overflow[fd], overflow[fd] + eol + 1);
			return (OK);
		}
	}
	while ((bytes_read = read(fd, buff, BUFFER_SIZE)))
	{
		...
		//printf("buff|%s|\n",buff);
		cpt += bytes_read;
		if((eol = find_pos_eol(buff))) //si buff et overflow ne sont pas vides
		{
			/*if (*buff == '\n')
				buff = &buff[1];*/
			//printf("eol:%zu\n", eol);
